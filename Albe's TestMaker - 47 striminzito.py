...
def add_header(document, title):
    ...
    printtitle = printtitlevalue.get()
    printvalue = headervalue.get()        
    
    if printtitle == 1 or printvalue == 1:
...
def add_to_test():
    ...
    justify = reading_test_justify_checkbox_value.get()
   ...
   # Prendi le risposte per il multiple choice tutte in blocco, splitta per riga e poi per virgola
    ...
    exercise_entry = {'task_description': task_description, 'underscore_lines': underscore_lines, 'reading_text': reading_text, 'sentence_fitb_mco':sentence_fitb_mco, 'list_style': list_style,
                      ...
                      'verticalmc' : verticalmc, 'hasdistractors' : hasdistractors, 'distractor_words' : distractor_words, 'justify' : justify,
    ...
    style_value.set(0)
    reading_test_justify_checkbox_value.set(0)
...
printtitlevalue = tk.IntVar()
printtitlevalue.set(1)

printtitlecheckbox = tk.Checkbutton (create_frame, text='Title:', width=20, anchor="e", variable = printtitlevalue)
printtitlecheckbox.grid(row=0, column=0, padx=5, pady=5, sticky='ew')
CreateToolTip(printtitlecheckbox, text = 'Should the title be printed?')
...
headerframe = tk.Frame(create_frame, bg="lightgreen")
headerframe.grid(row=8, column=0, columnspan=2)
headervalue = tk.IntVar()
headervalue.set(1)

headercheckbox = tk.Checkbutton (headerframe, text='Print header', width=10, anchor="e", variable = headervalue, bg="lightgreen")
headercheckbox.grid(row=0, column=1, padx=5, pady=5, sticky='ew')
CreateToolTip(headercheckbox, text = 'Should the header be printed?')


def update_languages():
    selected_language= language_dropdown_var.get()
    if selected_language == "English":
        student_name_string__entry.delete(0, tk.END)
        student_name_string__entry.insert(tk.END, "Name")
        student_class_string__entry.delete(0, tk.END)
        student_class_string__entry.insert(tk.END, "Class")
        date_string__entry.delete(0, tk.END)
        date_string__entry.insert(tk.END, "Date")
        exercise_name_string__entry.delete(0, tk.END)
        exercise_name_string__entry.insert(tk.END, "Exercise")
        true_string_entry.delete(0, tk.END)
        true_string_entry.insert(tk.END,"T")
        false_string_entry.delete(0, tk.END)
        false_string_entry.insert(tk.END, "F")
        
    elif selected_language == "Italiano":
        student_name_string__entry.delete(0, tk.END)
        student_name_string__entry.insert(tk.END, "Nome")
        student_class_string__entry.delete(0, tk.END)
        student_class_string__entry.insert(tk.END, "Classe")
        date_string__entry.delete(0, tk.END)
        date_string__entry.insert(tk.END, "Data")
        exercise_name_string__entry.delete(0, tk.END)
        exercise_name_string__entry.insert(tk.END, "Esercizio")
        true_string_entry.delete(0, tk.END)
        true_string_entry.insert(tk.END,"V")
        false_string_entry.delete(0, tk.END)
        false_string_entry.insert(tk.END, "F")
    elif selected_language == "Español":
        student_name_string__entry.delete(0, tk.END)
        student_name_string__entry.insert(tk.END, "Nombre")
        student_class_string__entry.delete(0, tk.END)
        student_class_string__entry.insert(tk.END, "Clase")
        date_string__entry.delete(0, tk.END)
        date_string__entry.insert(tk.END, "Fecha")
        exercise_name_string__entry.delete(0, tk.END)
        exercise_name_string__entry.insert(tk.END, "Tarea")
        true_string_entry.delete(0, tk.END)
        true_string_entry.insert(tk.END,"V")
        false_string_entry.delete(0, tk.END)
        false_string_entry.insert(tk.END, "F")
        ...
        student_name_string__entry.insert(tk.END, "Name")
        student_class_string__entry.delete(0, tk.END)
        student_class_string__entry.insert(tk.END, "Klasse")
        date_string__entry.delete(0, tk.END)
        date_string__entry.insert(tk.END, "Datum")
        exercise_name_string__entry.delete(0, tk.END)
        exercise_name_string__entry.insert(tk.END, "Übung")
        true_string_entry.delete(0, tk.END)
        true_string_entry.insert(tk.END,"R")
        false_string_entry.delete(0, tk.END)
        false_string_entry.insert(tk.END, "F")
    elif selected_language == "Français":
        student_name_string__entry.delete(0, tk.END)
        student_name_string__entry.insert(tk.END, "Nom")
        student_class_string__entry.delete(0, tk.END)
        student_class_string__entry.insert(tk.END, "Classe")
        date_string__entry.delete(0, tk.END)
        date_string__entry.insert(tk.END, "Date")
        exercise_name_string__entry.delete(0, tk.END)
        exercise_name_string__entry.insert(tk.END, "Exercice")
        true_string_entry.delete(0, tk.END)
        true_string_entry.insert(tk.END,"V")
        false_string_entry.delete(0, tk.END)
        false_string_entry.insert(tk.END, "F")
...
language_dropdown_var = tk.StringVar(headerframe)
language_dropdown_var.set("Layout language:")  # Default value

options = ["Layout language:","English", "Italiano", "Español", "Deutsch", "Français"]  # Add more options if needed
dropdown = ttk.OptionMenu(headerframe, language_dropdown_var, *options)
dropdown.grid(row=0, column=0)



# Call update_entries function when an option is selected
language_dropdown_var.trace_add("write", lambda *args: update_languages())
...
student_name_string_label = tk.Label (headerframe, text='Student:', width=20, height=1, anchor="e")
student_name_string_label.grid(row=1, column=0, padx=5, pady=5, sticky='ew')
student_name_string = tk.StringVar(create_frame)
student_name_string.set("Student")
student_name_string__entry = tk.Entry(headerframe, textvariable=student_name_string)
student_name_string__entry.grid(row=1, column=1, padx=5, pady=5, sticky='ew')

student_class_string_label = tk.Label (headerframe, text='Class:', width=20, height=1, anchor="e")
student_class_string_label.grid(row=2, column=0, padx=5, pady=5, sticky='ew')
student_class_string = tk.StringVar(create_frame)
student_class_string.set("Class")
student_class_string__entry = tk.Entry(headerframe, textvariable=student_class_string)
student_class_string__entry.grid(row=2, column=1, padx=5, pady=5, sticky='ew')

date_string_label = tk.Label (headerframe, text='Date:', width=20, height=1, anchor="e")
date_string_label.grid(row=3, column=0, padx=5, pady=5, sticky='ew')
date_string = tk.StringVar(headerframe)
date_string.set("Date")
date_string__entry = tk.Entry(headerframe, textvariable=date_string)
date_string__entry.grid(row=3, column=1, padx=5, pady=5, sticky='ew')

exercise_name_string_label = tk.Label (create_frame, text='Exercise:', width=20, height=1, anchor="e")
exercise_name_string_label.grid(row=9, column=0, padx=5, pady=5, sticky='ew')
...
exercise_name_string__entry.grid(row=9, column=1, padx=5, pady=5, sticky='ew')

true_string_label = tk.Label (create_frame, text='True:', width=20, height=1, anchor="e")
true_string_label.grid(row=10, column=0, padx=5, pady=5, sticky='ew')
...
true_string_entry.grid(row=10, column=1, padx=5, pady=5, sticky='ew')

false_string_label = tk.Label (create_frame, text='False:', width=20, height=1, anchor="e")
false_string_label.grid(row=11, column=0, padx=5, pady=5, sticky='ew')
...
false_string_entry.grid(row=11, column=1, padx=5, pady=5, sticky='ew')
...
CreateToolTip(fitb_label, text = 'Write here the words for Fill In The Blanks exercises.\n'
              'Separate the words or phrases with slash characters. This\nworks with sentences or with a text to read.')
...
readingtextframe = tk.Frame(exercise_frame, bg='lightgray')
readingtextframe.grid(row=4, column=0, padx=5, pady=5, sticky='ew')
# Reading Test (for Reading exercises)
reading_test_frame = tk.Frame(readingtextframe)
reading_test_frame.grid(row=0, column=0, padx=0, pady=0)
reading_test_label = tk.Label (reading_test_frame, text='Text to read:', width=30, height=1, anchor="center", bg='orange')
reading_test_label.grid(row=0, column=0, padx=0, pady=0, sticky='ew')
CreateToolTip(reading_test_label, text = 'The text will be printed right after the Task Description.\nCan also be used to give examples.')
reading_test_justify_checkbox_value = tk.IntVar()
reading_test_justify_checkbox_value.set(0)

reading_test_justify_checkbox = tk.Checkbutton(reading_test_frame, text="Justify", variable=reading_test_justify_checkbox_value, anchor="e", bg="orange")
reading_test_justify_checkbox.grid(row=0, column=1, padx=5, pady=5, sticky='ew')
CreateToolTip(reading_test_justify_checkbox, text = 'Recommended with walls of text.\nNot recommended with dialogues')


reading_test_entry = tk.Text(readingtextframe, height=7, width=35, bg='orange')
...
browse_frame = tk.Frame(create_frame)
browse_frame.grid(row=15, column=0, columnspan=2)

browse_button = tk.Button(browse_frame, text="Browse...", width= 8, command=open_file)
browse_button.grid(row=0, column=0, pady=5, padx=5)
CreateToolTip(browse_button, text = 'Opens an external .txt file containing previously exported exercises.')
...
catalogue_menu= ttk.Combobox(browse_frame, width=32)
catalogue_menu.grid(row=0, column=1, columnspan=2, padx=5, pady=5, sticky="ne")
catalogue_menu.bind("<<ComboboxSelected>>", display_exercise)



# Create a Treeview to display exercise data
exercises_catalogue = tk.Listbox(create_frame, height=12, width=30, selectmode=EXTENDED )
...
CreateToolTip(add_to_txt_button, text = 'Saves the current exercise to an external .txt file.\nThe exercise may later be easily retrieved and\npasted onto a new test.')
...
CreateToolTip(add_to_current_button, text = 'Copies the selected exercise to the exercise frame.')
