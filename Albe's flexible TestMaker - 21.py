...
def add_fitb(document, words):
    ...
    for triplet in words:
        print(triplet)  # Print each triplet
...
generate_button = tk.Button(create_frame, text="Generate", command=lambda: generate_test(title_entry.get(), filename_entry.get() + ".docx",
...
scramble_mc_checkbox = tk.Checkbutton(multiple_choice_options_frame, text="Scramble (disabled when creating\na text + multiple choice\nexercise with no sentences)", variable=scramble_mc_checkbox_value, anchor="e", bg="lawngreen")
...
add_button = tk.Button(exercise_frame, text="Add to test", width=20, command=add_to_test, bg="pink")
add_button.grid(row=7, column=0, padx=5, pady=5, sticky='ew')

add_to_txt_button = tk.Button(exercise_frame, text="Add to catalogue", width=20)
...
exercise_titles = extract_titles('exercises.txt')
catalogue_menu= ttk.Combobox(catalogue_frame, width=60, values=exercise_titles)
...
def copy_selected_items():
    ...
    selected_item = exercises_catalogue.curselection()
    if not selected_item:
        return

    selected_item = selected_item[0]
    
    # Get the corresponding exercise data
    exercise = exercises[selected_item]
    ...
    sentences = exercise.get('SENTENCES', [])
    for sentence in sentences:
        sentences_entry.insert(tk.END, sentence + '\n')
    ...
    global multiple_choice_options_value
    # Set the value of the global tk.IntVar variable for multiple_choice_options_value
    multiple_choice_options_value.set(exercise.get('MULTIPLE_CHOICE_OPTIONS_VALUE', 0))
    global underscore_lines_var
    # Set the value of the global tk.IntVar variable for underscore_lines_var
    underscore_lines_var.set(exercise.get('UNDERSCORE_LINES_VAR', 0))
    
    # Copy reading text to reading_text_entry
    reading_test_entry.insert(tk.END, exercise.get('READING_TEXT', ''))
